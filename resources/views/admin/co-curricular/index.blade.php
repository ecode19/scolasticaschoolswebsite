...
<section class="w-full p-4">
    <div class="w-full md:w-11/12 mx-auto flex items-center justify-between">
        <h2 class="text-2xl text-green-800">Co-Curricular Activities Management</h2>

        <button class="py-1 px-2 bg-green-700 hover:bg-gray-700 text-white rounded-md"><a
                href="{{ route('admin.cocurricular.create') }}"><i class="fa fa-plus mx-2"></i>New Activity</a></button>
    </div>
        ...
        <table class="w-full">
            ...
            <tbody>
                ...
                <tr class="text-gray-600">
                    ...
                    <td class="p-2 border border-gray-400">{{$activity->name}}</td>
                    <td class="p-2 border border-gray-400">{!!Str::words($activity->description, 20, '...') !!}</td>
                    <td class="p-2 border border-gray-400">{{$activity->academicLevel->name}}</td>
                    <td class="p-2 border border-gray-400">
                        <div class="flex items-center">
                        <a href="{{ route('admin.cocurricular.edit', $activity) }}"><button class="py-1 px-2 mx-2 bg-orange-600 hover:bg-gray-700 text-white rounded-md"><i
                            class="fa fa-pen mx-2"></i>Edit</button></a>
                        <form action="{{ route('admin.cocurricular.destroy', $activity) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button
