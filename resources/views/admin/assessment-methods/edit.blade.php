...
<section class="w-full p-4">
    <form action="{{ route('admin.assessment-method.update', $method) }}" method="POST" enctype="multipart/form-data"
        class="w-full md:w-10/12 mx-auto p-2 bg-green-100">
        @csrf
        @method('PUT')
        <h2 class="text-2xl text-center text-green-800 my-4">Update {{$method->name}} Details</h2>

        <div class="w-full flex flex-col md:p-4">
            <label for="name" class="w-full text-lg font-semibold text-gray-600">Method Name:</label>
        ...
        <div class="w-full flex flex-col md:p-4">
            <label for="description" class="w-full text-lg font-semibold text-gray-600">Method Description:</label>
        ...
        <div class="w-full flex flex-col md:p-4">
            ...
            <select name="academic_level_id"
                ...
                @foreach ($academicLevels as $level)
                    <option value="{{ $level->name }}" {{ $level->name === $methodLevel->name ? 'selected' : '' }}>{{ $level->name }}</option>
                @endforeach
            </select>
        ...
        <div class="w-fit mx-auto my-4">
            <button class="py-1 px-2 text-lg bg-green-700 hover:bg-gray-700 text-white rounded-md"><i
                    class="fa fa-pen mx-2"></i>update method</button>
        </div>
    </form>
